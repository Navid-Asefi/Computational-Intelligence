...
def trignometric2(x):
    n = len(x)
    return np.sum([(n - np.sum(np.cos(x)) + i*(1-np.cos(x[i])))**2 for i in range(n)])
...
benchmark_info = {
    ...
    "quartic_noise": {"func": quartic_noise, "bounds": (-1.28, 1.28), "default_dim": 30},
    ...
    "xin_she_yang": {"func": xin_she_yang, "bounds": (-2*np.pi, 2*np.pi), "default_dim": 30},
    ...
    "wayburn_seader3": {"func": wayburn_seader3, "bounds": (-500, 500), "default_dim": 2},
    ...
    "xin_she_yang2": {"func": xin_she_yang2, "bounds": (-2*np.pi, 2*np.pi), "default_dim": 30},
    ...
    "xin_she_yangN2": {"func": xin_she_yangN2, "bounds": (-2*np.pi, 2*np.pi), "default_dim": 30},
...
def create_results_table(results):
    ...
    # Create table
    print("\n" + "="*80)
    print(f"{'Benchmark Function':<30} {'Best':<15} {'Mean':<15} {'Std':<15} {'Rank':<5}")
    print("="*80)
    ...
    for rank, (name, stats) in enumerate(sorted_results, 1):
        if np.isnan(stats["min"]):
            ...
            print(f"{name:<30} {stats['min']:<15.6e} {stats['mean']:<15.6e} {stats['std']:<15.6e} {rank:<5}")
